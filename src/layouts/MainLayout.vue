  ...
  <q-layout view="hHh Lpr lFf" v-else>
    ...
    >
      <q-list>
        ...
        <q-item to="/sensors" active-class="q-item-no-link-highlighting">
          <q-item-section avatar>
            <q-icon name="settings_remote" />
          </q-item-section>
          <q-item-section>
            <q-item-label>Sensörler</q-item-label>
          </q-item-section>
        </q-item>
...
import { pharmacies } from "../api/pharma.js";
...
export default {
  ...
  created() {
    this.$auth.$watch("loading", loading => {
      pharmacies().then(data =>
        console.log((this.pharmacyName = data.data.Name))
      );
    });
  },
  methods: {
